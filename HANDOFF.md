...
**Sesión 2026-06-01 — accesos de cliente al portal, fix de crash del API, hardening, cutover de correo a mailcow y aprobación de cotización desde el portal.**
...
- **Test pendiente del portal-aprobación**: dejé `COT-2026-0001` (expediente 1, cliente_id 2) en estado `enviada` para que Pablo pruebe en el browser logueado como `paulette@gmail.com` (login de portal de ese cliente). Para revertir: `UPDATE comercial.cotizaciones SET estado='borrador' WHERE id=1;`. ⚠️ Dato inconsistente preexistente: el hito ya estaba en `aprobacion_cliente` con la cot en `borrador` (data demo); el gating (cot `enviada`) lo maneja bien.
