...
- ✅ **Auditoría de seguridad (agente `security-auditor`) — postura madura, 0 críticas/altas.** Verificado OK: SQL 100% parametrizado, authz granular en todas las rutas, aislamiento por `cliente_id` en el portal, sin secretos hardcodeados, cookies/CSRF/rate-limit correctos. **Fixes aplicados:** **M-1** (XSS en plantillas de email) → `escapeHtml`/`escapeHtmlMultiline` en `services/email.ts` (todos los templates) + `routes/informes-tecnicos.ts`; verificado con payload `<img onerror>`/`<script>`. **B-2** (CORS) → guard en `config/env.ts`: en `production`, `CORS_ORIGINS` es obligatorio (prod ya lo setea, no rompe). **NO aplicados (a propósito):** **B-1** (bindear API/web a `127.0.0.1`) — riesgoso: el browser usa `api.techtrafo.com` vía el edge `.7` + `techtrafo-nginx`(8080)/`web-nginx`(443); hay que trazar el ruteo y **probar con rollback** antes. **M-2** (PDFs internos en `routes/pdf.ts` sin chequeo de pertenencia — acceso horizontal entre staff, NO entre clientes) → **DECIDIDO 2026-06-07: acceso amplio del staff es INTENCIONAL** (riesgo aceptado por Pablo, no se segmenta; el detalle ya se limita por rol vía `resolverNivel`). Bajas pendientes menores: B-3 (Grafana SMTP skip-verify), B-4 (nginx 50M vs Express 1M/20M).

**Pendientes abiertos al cierre:**
- Pablo debe **cambiar su contraseña temporal** del panel.
- **[Seguridad] B-1** (opcional, con prueba): rutear todo el tráfico por nginx y bindear `3000`/`3002` a loopback en `docker-compose.yml`. (M-2 ya decidido: acceso amplio intencional.)
